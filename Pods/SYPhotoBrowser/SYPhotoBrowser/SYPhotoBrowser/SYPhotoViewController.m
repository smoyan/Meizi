...
@property (nonatomic, strong) NSMutableArray<UIButton *> *markButtons;
...
- (void)createMarkButtons
{
    if (!self.markButtons) {
        CGFloat gap = self.view.frame.size.width / 6;
        CGFloat side = 50;
        self.markButtons = [NSMutableArray arrayWithCapacity:5];
        for (int i = 1; i <= 5 ; i ++) {
            ...
            [self.markButtons addObject:btn];
        }
    }
}

- (void)onMarkButtonClick:(UIButton *)button
{
    NSString *imageName = [_imageSource componentsSeparatedByString:@"/"].lastObject;
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://10.66.119.102:1234/%@?%d", _imageSource, button.tag]];
